...
*/
	...
	class getMsgRou{
		...
		public function getRouteMessage($json) {
          ...
          //
          // JRICO: DISEABLE limit time
          //
					$sql_is_own = "SELECT  rmsg.id as msg_Id, rusg.idRoutePoint, rmsg.message,rusg.idUsrRoute,rusg.idUsr,rusg.time FROM `routemessages` as rmsg, routeusage as rusg WHERE rmsg.idRouteUsage=rusg.id AND rusg.idUsrRoute = ".$json['route_id']." AND  rusg.idUsr = ".$this->userId." Order by rusg.idRoutePoint ";
          ...
          //  JRICO: DISEABLE limit time
          //
					$sql_is_own ="SELECT rmsg.id as msg_Id,rusg.idRoutePoint, rmsg.message,rusg.idUsrRoute,rusg.idUsr,rusg.time FROM `routemessages` as rmsg, routeusage as rusg WHERE rmsg.idRouteUsage=rusg.id AND rusg.idUsrRoute =".$json['route_id']."  Order by rusg.idRoutePoint";
				}
            ...
            //
            // DISABLE return, now return message ID
            //
            $arrData[] = (int)$arrSelect_route[$kk]['msg_Id'];

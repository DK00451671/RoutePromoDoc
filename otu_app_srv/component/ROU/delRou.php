	...
	class delRou{
		...
		function deleteUserRoute($json) {
			...
			if(empty($route)) {
				...
				// check the route is shared or not
				$sql_route_share = "SELECT  `idUsrRoutes`, `idUsr`, `idUsr_ToShareRouteWith`, `route_owner`, `valid` FROM `sharedroutes` WHERE `idUsr_ToShareRouteWith`=".$this->userId."  AND `idUsrRoutes`=".$json['route_id'];
				$route = $this->exec_query($sql_route_share,'SEL');
				
				if(isset($route['idUsrRoutes']) && !empty($route)) { 
						$sql_update_shared = "UPDATE `sharedroutes` SET `valid`=0 WHERE `idUsr_ToShareRouteWith`=".$this->userId."  AND `idUsrRoutes`=".$json['route_id'];
			...
			}else
			{
			    ...
			    $route = $this->exec_query($sql_update,'UPD');
				return TRUE;
			}
		}

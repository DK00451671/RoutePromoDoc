	...
	class accShareRou{
		...
		public function activeSharedRoute($json) {
			...
			if(count($arrSelect_route)>0 && !empty($arrSelect_route)){
				$sql_update = "UPDATE `sharedroutes` SET  `accepted`=1,`valid`=1 WHERE `idUsr_ToShareRouteWith`=".$this->userId." AND `idUsrRoutes`=".$json['route_id'];
		...
		*/
		...
		public function validarEstructura($jsonEntrada) {
				...
				if($this->validateData($json)) {
					...
					if(!empty($arrResult)) {
						if(isset($arrResult['is_non_register']) && !$arrResult['is_non_register'] ) { 
							$this->userId = $arrResult['id'];
							$this->activeSharedRoute($json);
						} else {
							$this->arrResultData['success'] = 'false';
							$this->arrResultData['m'] = 'non-register user, please update signIn';
						}
					}
					else {
						$this->arrResultData['success'] = 'false';

...
*/
	...
	class getMyRouLst{
		...
		public function getUserRoute($userId){
			...
			$route_sql = "SELECT `id`, `route_name`, active FROM `usrroutes` WHERE `idUsr`=".$userId . " ORDER BY id DESC ";	
			$arruserResult = $this->exec_query($route_sql,'ALL');
			
					
			$shared_rou_sql = "SELECT rutcnt.shared_count as shared_times, srou.accepted, usr.id as share_route_from_id,usr.user_name as share_route_from, urou.id as route_id ,urou.route_name, srou.date as shared_date, srou.valid FROM `usrroutes` as urou , sharedroutes as srou , usr as usr , routecount as rutcnt WHERE rutcnt.route_id = srou.idUsrRoutes AND urou.id = srou.idUsrRoutes AND srou.idUsr_ToShareRouteWith=".$userId." AND usr.id=srou.idUsr AND srou.valid=1";
			...
			foreach($arrShareRouResult as $kk=>$val) {
			  $sql_rou_owner = "SELECT usr.id as owner_id , usr.user_name as owner_name FROM usr as usr , usrroutes as urut WHERE urut.idUsr = usr.id AND urut.id=".$arrShareRouResult[$kk]['route_id'];
				...
				$arrShareRouResult[$kk]['owner_id'] = (int)$arrShareRouOwn['owner_id'];
			...
			if(is_array($arruserResult) && count($arruserResult)>0) {
				...
				foreach($arruserResult as $kk=>$val) {
					$this->arrResultData['m']['own'][$cntOwnInx][$val['route_name']] = (int)$val['id'];
					$this->arrResultData['m']['own'][$cntOwnInx]['active'] = (int)$val['active'];
					$cntOwnInx++;
				}
			}else
			...
			if(is_array($arrShareRouResult) && count($arrShareRouResult)>0) {
			  $cntInx = 0;
				foreach($arrShareRouResult as $kk=>$val) {
					$this->arrResultData['m']['shared'][$cntInx]['route_id'] = (int)$val['route_id'];
					$this->arrResultData['m']['shared'][$cntInx]['route_name'] = $val['route_name'];
				    $this->arrResultData['m']['shared'][$cntInx]['owner_id'] = (int)$val['owner_id'];
					$this->arrResultData['m']['shared'][$cntInx]['owner_name'] = $val['owner_name'];
					$this->arrResultData['m']['shared'][$cntInx]['share_route_from_id'] = (int)$val['share_route_from_id'];
					...
					$this->arrResultData['m']['shared'][$cntInx]['shared_times'] = (int)$val['shared_times'];
					$this->arrResultData['m']['shared'][$cntInx]['accepted'] = (int)$val['accepted'];
					$this->arrResultData['m']['shared'][$cntInx]['valid'] = (int)$val['valid'];

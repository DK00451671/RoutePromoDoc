		...
		public function getRouteInfo($json) {
			...
			if(!empty($arrSelect_route['idUsr']) && is_array($arrSelect_route) && !empty($arrSelect_route)) {
			    ...
			    $arrSelect_route_count = $this->exec_query($select_route,'SEL');
          	    ...
          	    $arrData['times_shared'] = (int)$arrSelect_route_count['shared_count'];
          	    $arrData['counter_usage'] = (int)$arrSelect_route_count['usages_count'];
          	    $arrData['share_with'] = (int)$arrSelect_route_count['share_with'];
			    ...
			    $select_route = "SELECT count(promotionAssign.idPromotions) total, GROUP_CONCAT(promotionAssign.idPromotions) proms_id FROM promotionAssign join promotions on (promotions.id = promotionAssign.idPromotions) where idRoute = ".$json['route_id']." and idUsr = ".$this->userId." AND promotionAssign.valid =1 and promotions.endDate > now() and promotions.cancelDate is null";
			    $arrSelect_proms = $this->exec_query($select_route,'SEL');

          	    $arrData['promotions_assign'] = (isset($arrSelect_proms['proms_id']) &&  $arrSelect_proms['proms_id'] != NULL) ? $arrSelect_proms['proms_id'] : array() ;
          	    $arrData['promotions_counter'] = (int)$arrSelect_proms['total'];

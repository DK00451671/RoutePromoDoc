...
*/
	...
	class accFri{
		...
		public function acceptFriendRequest($json) {
		   
		   //Check friend request present or not
		   
		   $sql_check = "SELECT `id`, `date`, `accepted`, `idUsr`, `idFriend`, `valid` FROM `friends` WHERE `idFriend`=".$this->userId." AND `idUsr`=".$json['user_id'] ;
		   $arrChkData = $this->exec_query($sql_check,'SEL');
		  		 
		   if(!empty($arrChkData) && isset($arrChkData)){
		        //set accepted and valid flag as 1
		     	$sql_insert = "UPDATE `friends` SET `accepted`=1 WHERE idUsr=".$json['user_id']." AND idFriend=".$this->userId;
			...
			/*else { // temp commit this code . un commit if needed
				$sql_check_frnd = "SELECT `id`, `date`, `accepted`, `idUsr`, `idFriend`, `valid` FROM `friends` WHERE `idFriend`=".$this->userId." AND `idUsr`=".$json['user_id'] ;
				$arrChkData_frnd = $this->exec_query($sql_check_frnd,'SEL');
		  
				if(!empty($arrChkData_frnd) && isset($arrChkData_frnd)){
					//set accepted and valid flag as 1
					$sql_insert = "UPDATE `friends` SET `accepted`=1 WHERE idFriend=".$this->userId." AND idUsr=".$json['user_id'];
					$this->exec_query($sql_insert,'UPD');
					$this->arrResultData['m'] = 'ok';
				}

...
*/
	...
	class shareRou{
		...
		public function checkRegAndNonRegUsr($json)
        {
			...
			$share_rou_to = $json['share_rou_to'];
        ...
        if(count($arrShareRou) < 1) {
            ...
            // insert token
            //
			$sql_share_route = "INSERT INTO `sharedroutes`(`id`, `idUsrRoutes`,`idUsr`, `idUsr_ToShareRouteWith`,`route_owner`, `date`, `accepted`,`token`) VALUES (0,".$json['idUsrRoutes'].",".$json['user_id'].",".$json['idUsr_ToShareRouteWith'].",".$json['route_own'].",'".date('Y-m-d H:i:s')."',0,'".$this->token."')";
		...
		*/
				...
				if($this->validateData($json)) {
					...
					if(!empty($arrResult)){
							if($this->shareRoute($arrResult)){
							   $this->arrResultData['success'] = 'true';
                               $this->arrResultData['m'] = array(
                                                             "status"=>'Share route successfully',
                                                             "token" => $this->token);
							}
					}
					else{
		...
		function validateData($json) {
			...
			if(!array_key_exists("account_type",$json) || empty($json['account_type'])) {
			 ...
			 if(!array_key_exists("share_rou_to",$json) || empty($json['share_rou_to'])) {
				$this->arrResultData['success'] = 'false';
				$this->arrResultData['m'] = 'share route id is missing';
				   if($this->debug)
						$this->arrResultData['debug'][] = array("share_rou_to"=>"share_rou_to is missing or Invalid index");
					$flag=1;
			 }else
			 {
				if(is_integer($json["share_rou_to"])) {
					$this->arrResultData['success'] = 'false';
					$this->arrResultData['m'] = 'Invalid share_rou_to value';
					if($this->debug)
						$this->arrResultData['debug'][] = array("share_rou_to"=>"Invalid share_rou_to value must be proper format");
					$flag=1;
				}
				else {
